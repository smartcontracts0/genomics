...
contract SequencedGenomicDataNFTs is ERC721URIStorage, Ownable{
    ...
    function mint(string memory _tokenURI, uint256 _parentID) external returns(uint) {
        require(GenomicsDataManagement.SequencedNFTMintingBalance(msg.sender,_parentID) > 0, "The user is not eligible for minting"); //This implicitly requires the msg.sender to be the owner of the parentID
